...
**Last updated:** 2026-06-01 (v0.4.0-preview)
**Branch:** main

## OVERVIEW

SwiftUI macOS menu-bar app + Rust CLI rewriter. Three input gestures feed one rewrite pipeline, plus a fourth experimental path that drives inline autocomplete:
...
4. **Typing** (Autocomplete, default off) → read text-before-caret + caret bounds → tiny model (`qwen2.5:1.5b`) → grey ghost text near the caret → **Tab** accepts (feat-013, experimental; caret positioning is under active debug).
...
| Caret geometry (autocomplete) | `Owlet/Owlet/AXBridge.swift` → `readCaretContext` / `caretCocoaRect` | Reads text-before-caret + caret bounds (`kAXBoundsForRange`), flips Quartz→Cocoa coords. **Has TEMP diagnostic logging (`caretgeom`) for the active ghost-position bug; remove after fix** |
| Autocomplete controller | `Owlet/Owlet/AutocompleteController.swift` | @MainActor brain: 120 ms debounce, cancels superseded predictions, password/empty guards, `accept()`/`dismiss()` |
| Prediction client | `Owlet/Owlet/Predictor.swift` | `Predicting` protocol + `OllamaPredictor` (`/api/generate`, `qwen2.5:1.5b`, short raw completion) |
| Ghost-text overlay | `Owlet/Owlet/GhostTextOverlay.swift` | Click-through `.nonactivatingPanel` drawn at the (Cocoa-converted) caret rect |
...
| Preferences | `Owlet/Owlet/Preferences.swift` | UserDefaults, `.hotkey \| .launchAtLogin \| .model \| .visionModel \| .autocompleteEnabled \| .autocompleteModel` changes |
...
# Rust + Ollama smoke (needs ollama serve + qwen2.5:1.5b)
...
- `install.sh` pulls a single shared model (`qwen2.5:1.5b`) for both rewriter and autocomplete (feat-014). The vision model (`llava:7b`) for feat-006 must be pulled manually.

## CURRENT STATE / FOLLOW-UPS (as of 2026-06-01)

- **feat-001..008 `done`/merged** (v0.4.0-preview): rewrite core, settings, hold-Option button, screenshot rewrite, URL preservation (feat-007), per-rewrite context (feat-008).
- **feat-013 (inline autocomplete): IN PROGRESS, EXPERIMENTAL.** The prediction loop, Tab/Esc handling, and password/empty guards are implemented and unit-tested (the AX bridge is mocked, so runtime accept/dismiss are not yet observed). **Known blocker:** the ghost text renders at the wrong screen position (away from the caret) even in AX-native fields like TextEdit. The Quartz→Cocoa coordinate flip in `AXBridge.cocoaRect(fromAXRect:)` was added but did not resolve it on a single Retina display, so `caretCocoaRect` has **temporary `caretgeom` diagnostic logging** to capture the raw vs converted rects. Next step: read that log (`log show --predicate 'subsystem == "co.greenpassport.owlet" AND category == "caretgeom"'`) after typing in TextEdit, then fix the mapping and remove the diagnostic. Don't describe autocomplete as working.
- **feat-009..012 (rewriter dispatcher + event extraction): PAUSED**, not deleted, behind the autocomplete pivot. Design doc stays valid; resume after feat-013/014.
- **feat-005 (hold-Option button) & feat-006 (screenshot rewrite): MANUAL VERIFY PENDING** on a multi-monitor setup. Unit-tested, but the secondary-display capture path is not human-confirmed. Don't describe them as verified-working.
...
- **Single-model consolidation (feat-014) done.** Rewriter default + `install.sh` now use `qwen2.5:1.5b` (one ~1 GB download shared with autocomplete), gated on the `tests/smoke.sh` prompt-hardening checks (rewrites-not-answers, never-echoes-context).
